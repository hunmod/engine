...
$allegrenek=$gmrec_class->get_list('allergen',array('status'=>2));

//arraylist ($allegrenek);
...
$status=$gmrec_class->status();
$sorrend=$gmrec_class->sorrend();
$form=new formobjects();
...
if ($_POST['hirsave']=='1'){
$hirid=$gmrec_class->save($_POST); 
$_POST["id"]=$hirid;
//$gmrec_class->save_ad_tags_field($_POST);
//echo $hirid;
...
if ($hirid>0){
	$filters['id']=$hirid;
	$filters['status']="all";
	$news=$gmrec_class->get($filters,$order='',$page='all');
	$adat=$news['datas'][0];
	$nimg=$gmrec_class->getimg($adat['id']);
	$tags=$gmrec_class->get_ad_tag(array("ad"=>$filters["id"],"active"=>'all'));
...
$menuk=$MenuClass->menu_selectboxfilter($menustart,array("modul"=>"gmrec"),$filtersm,$order='',$page='all');
...
<br />
	...
	<?php echo $lan['osszetevok'];?>:
    <?php  $form->kcebox("osszetevok",$Text_Class->htmlfromchars($adat["osszetevok"])) ?>
        <br />
    <?php echo $lan['leiras'];?>:<?php  $form->kcebox("leiras",$Text_Class->htmlfromchars($adat["leiras"])) ?>
        <br />   
        <?php echo $lan['subject'];?>:    <?php  $form->kcebox("subject",$Text_Class->htmlfromchars($adat["subject"])) ?>
        <br />
	<?php echo $lan['allergen'];?>:

<?php
foreach ($allegrenek as $allergen){?>
<input name="addonid[<?= $allergen['id'];?>]" type="checkbox" value="<?= $allergen['id'];?>" <?php if ($adat["addonid"][$allergen['id']]) { ?>checked="checked"<?php } ?> /><?= $allergen['name'];?>
<?php	}?>        
<br />
