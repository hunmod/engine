...
//arraylist($datas);
    ...
    public function checkbox($name, $value, $caption = "", $class = "checkbox-inline")
    {
        ...
        echo '<label class="' . $class . '"><input type="checkbox" name="' . $name . '" value="1" ' . $status . '>' . $caption . '</label>';

    }

    public function radiobox($name, $value, $caption = "", $class = "checkbox-inline", $chk)
    {
        if ($chk) $status = ' checked';

        //echo '<label class="'.$class.'" title="'.$caption.'"><input type="checkbox" name="'.$name.'" value="1" '.$status.'>'.$caption.'</label>';
        echo '<label class="' . $class . '"><input type="radio" name="' . $name . '" value="' . $value . '" ' . $status . '>' . $caption . '</label>';
	...
	['Bold','Italic','Underline','Format','Font','FontSize'],['TextColor','BGColor']],";
        ...
        $kcescript .= "language: 'hu',uiColor: '" . $editorcolor . "',filebrowserImageBrowseUrl : '" . $homeurl . "/scripts/ckfinder/ckfinder.html'});</script>
...
";
    ...
    public function numbox($name, $value, $caption = "", $class = "control-label", $requied = 0, $min = NULL, $max = NULL)
    {
        ...
        echo '<label for="inputHelptext" class="' . $class . '" id="label_' . $name . '">' . $caption . '</label>' .
            '<input type="number" name="' . $name . '" id="' . $name . '" value="' . $value . '" onKeyUp="numericFilter(' . "\'" . $name . "\'" . ');"  placeholder="' . $caption . '"' . $reqval . ' class="form-control" />';
//echo '<input type="text" name="'.$name.'" id="'.$name.'" value="'.$value.'" onchange="$("#'.$name.'").ForceNumericOnly();" />';
    ...
    {
        global $selected;
        echo '<label for="' . $name . '">' . $caption . '</label>' . '<select class="' . $class . '" name="' . $name . '" id="' . $name . '">';
...
//if ($_GET["ip"]){echo $_GET["ip"];} //else {echo dateprintshort($date);}
        $ret = '<input type="date" name="' . $name . '" id="' . $name . '"  value="' . $ertek[0] . '" />';
