...
$list_term_mezok_text='nev,suly,ar,ar_vat,ar_beszerzes,kod,kvn_kod,kvn_teszor';
$list_term_mezok=explode(',',$list_term_mezok_text);
...
if (page_settings("term_maxitem")!="")
{$maxegyoldalon=page_settings("term_maxitem");}
...
arraylist($almenu);*/
	...
	if ($_GET['ord']!=""){
		$order="`".$_GET['ord']."` ";
		if ($_GET['ord_a']!=""){$order.=$_GET['ord_a'];}
		else {$order.='ASC';}
	}


$datas=$class_termekek->get($filters,$order,$page='all');
//arraylist($datas);
$elemek=$datas["datas"];
$page_keywords="";
$page_ogimage="";
