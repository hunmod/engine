...
function loadblogajax(){
          $.getJSON(''+serverurl+'service.php?q=page/ajax_taglist&tag='+$("#blogtag").val()+'', function(data) {
			$("#blogtagsl").empty();
			if(data.length > 0) {
				//$('#tagsave').attr("disabled", "disabled");
			}
...
$status=$page_class->status();
$sorrend=$page_class->sorrend();
$form=new formobjects();
...
if ($_POST['hirsave']=='1'){
$hirid=$page_class->save($_POST); 
$_POST["id"]=$hirid;
//$page_class->save_ad_tags_field($_POST);
//echo $hirid;
...
if ($hirid>0){
	$filters['id']=$hirid;
	$filters['status']="all";
	$news=$page_class->get($filters,$order='',$page='all');
	$adat=$news['datas'][0];
	$nimg=$page_class->getimg($adat['id']);
	$tags=$page_class->get_ad_tag(array("ad"=>$filters["id"],"active"=>'all'));
...
<div id="desc_hu">
	<?php echo $lan['cim'];?>:
	<input name="cim_hu" id="cim_hu"  type="text" value="<?php echo $Text_Class->htmlfromchars($adat["cim_hu"]); ?>"  maxlength="200" style="  width: 217px;" /><br />
	<?php echo $lan['innertext'];?>:
    <?php  $form->kcebox("hir_hu",$Text_Class->htmlfromchars($adat["hir_hu"])) ?>
        <br />
    <?php echo $lan['outertext'];?>:    <?php  $form->kcebox("hir2_hu",$Text_Class->htmlfromchars($adat["hir2_hu"])) ?>
        <br />
</div>		
<div id="desc_en">
	<?php echo $lan['cim'];?>:
	<input name="cim_en" id="cim_en"  type="text" value="<?php echo $Text_Class->htmlfromchars($adat["cim_en"]); ?>"  maxlength="200" style="  width: 217px;" /><br />
	<?php echo $lan['innertext'];?>:
    <?php  $form->kcebox("hir_en",$Text_Class->htmlfromchars($adat["hir_en"])) ?>
        <br />
    <?php echo $lan['outertext'];?>:    <?php  $form->kcebox("hir2_en",$Text_Class->htmlfromchars($adat["hir2_en"])) ?>
        <br />
</div>		
